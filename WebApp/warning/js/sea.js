...
/*! Sea-css.js 3.0.0 | seajs.org/LICENSE.md */
...
//宝驾JS补丁和公用方法
function AddNewTab(id,name,gourl,isTreeMenu){
	if(!id || id ==""){
		seajs.alert("跳转页面参数Id未设置");
		return false;
	}
	if(!name || name ==""){
		seajs.alert("跳转页面参数name未设置");
		return false;
	}
	if(!gourl || gourl ==""){
		seajs.alert("跳转页面参数gourl未设置");
		return false;
	}

	if(parent){
		parent.openTab(id,name,gourl,false)
	}
}

function openTab(id,name,gourl,isTreeMenu){
    var menutabs = avalon.vmodels.menutabs
    var opens = menutabs.tabs;
    if(opens && opens.length> 10)
    {
       seajs.alert("您当前打开的页面过多，请先关闭不必要的页面");
       return false;
    }

    if(!isTreeMenu)
    {
        var cur = opens[avalon.vmodels.menutabs.active]
        id = cur.id + "-" + id;
    }         
    if(gourl.indexOf("http:")<0){
        gourl = (sysConfig.viewPath + gourl)
    }

    if(opens && opens.length>0 )
    {
        for (var i = 0; i < opens.length; i++) {
            if(opens[i].id == id && opens[i].title == name){
                menutabs.active =i;
                //gourl =document.getElementById("bjiframe"+id).src;
                menutabs.tabpanels[i].content='';
                menutabs.tabpanels[i].content= FConfig.iframeHtml.Format(id,FConfig.iframeH,gourl);
                return false;
            }
        };
    }
    ...
    var curpage = {
        ...
        removable: true,
        remove:"remove",
        content:FConfig.iframeHtml.Format(id,FConfig.iframeH,gourl)
    }

    menutabs.add(curpage);
    menutabs.active =opens.length -1;
}

function goSingin(){
    window.location.href=sysConfig.signinUrl
}

seajs.alert=function(txt){
	var validationtip;
	 seajs.use(["bj-tip", "bj-tip-css4"], function(tip) {
	    if (validationtip == undefined) validationtip = new tip();
	    validationtip.alert({txt: txt});
	})
}
...
seajs.config({
    alias: {
        "$": "A/jquery-2.1.1.min",
        "A": "A/avalon.min",
        "C": "A/js/widget/common/common",

        "browser":  "A/js/widget/common/browser",
        "cookie":   "A/js/widget/common/cookie",
        "store":    "A/js/widget/common/store",
        "promise":  "A/js/widget/common/promise",

        "A-getModel":       "A/js/libs/avalon.getModel",
        "A-live":           "A/js/libs/avalon.live",
        "A-position":       "A/js/libs/avalon.position",
        "A-scrollspy":      "A/js/libs/avalon.scrollspy",
        "A-button":         "A/js/libs/avalon.button",
        "A-checkboxlist":   "A/js/libs/avalon.checkboxlist",
        "A-verify":         "A/js/libs/avalon.validation",
        "A-dialog":         "A/js/libs/avalon.dialog",
        "A-doublelist":     "A/js/libs/avalon.doublelist",
        "A-drag":           "A/js/libs/avalon.draggable",
        "A-dropdown":       "A/js/libs/avalon.dropdown",
        "A-dropdownlist":   "A/js/libs/avalon.dropdownlist",
        "A-flipswitch":     "A/js/libs/avalon.flipswitch",
        "A-menu":           "A/js/libs/avalon.menu",
        "A-tab":            "A/js/libs/avalon.tab",
        "A-timer":          "A/js/libs/avalon.timer",
        "A-tree":           "A/js/libs/avalon.tree",
        "A-tree-check":     "A/js/libs/avalon.tree.check",
        "A-tree-droptree":  "A/js/libs/avalon.tree.droptree",
        "A-tree-edit":      "A/js/libs/avalon.tree.edit",
        "A-tree-menu":      "A/js/libs/avalon.tree.menu",
        "A-tooltip":        "A/js/libs/avalon.tooltip",
        "A-datepicker":     "A/js/libs/avalon.datepicker",
        "A-datepicker-lang":"A/js/libs/avalon.datepicker.lang",
        "A-daterangepicker":"A/js/libs/avalon.daterangepicker",
        "A-coupledatepicker": "A/js/libs/avalon.coupledatepicker",
        "A-loading":        "A/js/libs/avalon.loading",
        "A-scrollbar":      "A/js/libs/avalon.scrollbar",
        "A-slider":         "A/js/libs/avalon.slider",
        "A-request":        "A/js/libs/avalon.request",
        "A-suggest":        "A/js/libs/avalon.suggest",
        "A-textbox":        "A/js/libs/avalon.textbox",
        "A-pager":          "A/js/libs/avalon.pager",
        "A-smartgrid":      "A/js/libs/avalon.smartgrid",

        "W":        "A/js/widget/widget",
        "bug":      "A/js/widget/common/bug",
        "bj-tip":   "A/js/widget/bj.tip",
        "bjPage":   "A/js/widget/bjPage"
    },
...
seajs.config({
    alias: {
        "bj-tip-css2":          "A/css/widget/bj.tip.css",
        "bj-tip-css4":          "A/css/widget/bj.tip.css",
        "A-common-css":         "A/css/libs/oniui-common.css",
        "A-button-css":         "A/css/libs/avalon.button.css",
        "A-checkboxlist-css":   "A/css/libs/avalon.checkboxlist.css",
        "A-scrollbar-css":      "A/css/libs/avalon.scrollbar.css",
        "A-slider-css":         "A/css/libs/avalon.slider.css",
        "A-dialog-css":         "A/css/libs/avalon.dialog.css",
        "A-doublelist-css":     "A/css/libs/avalon.doublelist.css",
        "A-dropdown-css":       "A/css/libs/avalon.dropdown.css",
        "A-dropdownlist-css":   "A/css/libs/avalon.dropdownlist.css",
        "A-menu-css":           "A/css/libs/avalon.menu.css",
        "A-flipswitch-css":     "A/css/libs/avalon.flipswitch.css",
        "A-tree-css":           "A/css/libs/avalon.tree.css",
        "A-tree-menu-css":      "A/css/libs/avalon.tree.menu.css",
        "A-tree-droptree-css":  "A/css/libs/avalon.tree.droptree.css",
        "A-tab-css":            "A/css/libs/avalon.tab.css",
        "A-tooltip-css":        "A/css/libs/avalon.tooltip.css",
        "A-timer-css":          "A/css/libs/avalon.timer.css",
        "A-datepicker-css":     "A/css/libs/avalon.datepicker.css",
        "A-daterangepicker-css":"A/css/libs/avalon.daterangepicker.css",
        "A-coupledatepicker-css":"A/css/libs/avalon.coupledatepicker.css",
        "A-loading-css":        "A/css/libs/avalon.loading.css",
        "A-suggest-css":        "A/css/libs/avalon.suggest.css",
        "A-textbox-css":        "A/css/libs/avalon.textbox.css",
        "A-pager-css":          "A/css/libs/avalon.pager.css",
        "A-smartgrid-css":      "A/css/libs/avalon.smartgrid.css"
    }
});