...
define( function(require) {
    ...
    if (sysConfig.jsTracking) avalon.log('seajs-Tracking："doublelist --- doublelist"'); //

    require("A-common-css");
    require("A-doublelist-css");

    var template, dataTpl;
    //读取template
    var templates = require("text!../libs/model/avalon.doublelist.html");
    ...
    var widget = avalon.ui.doublelist = function(element, data, vmodels) {
        ...
        var vmodel = avalon.define(data.doublelistId, function(vm) {
            ...
            vm._updateAll = function($event, addOrDelete) {
                var tar = addOrDelete === "delete" ? vm.select : vm.data
                if(tar.length == 0) return
                if(!vmodel.countLimit(vmodel.select, addOrDelete, tar.length)) return
                if(addOrDelete === "delete") {
                    for(var i = 0, len = tar.length; i < len; i++) {
