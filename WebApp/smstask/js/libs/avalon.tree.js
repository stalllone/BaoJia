...
define( function(require) {
    ...
    if (sysConfig.jsTracking) avalon.log('seajs-Tracking："tree --- 树"'); //

    require("A-common-css");
    require("A-tree-css");

    var template, leafTemplate, parentTemplate, nodesTemplate;
      //读取template
    var templates = require("text!../libs/model/avalon.tree.html");
    templates = templates.split("MS_OPTION_TEMPLATE");
    template = templates[0];
    leafTemplate = templates[1];
    parentTemplate = templates[2];
    nodesTemplate = templates[3];
    ...
    function simpleDataToTreeData(arr, vm) {
      ...
      *  ]
      */
    function  transformTozTreeFormat (sNodes,vm) {
        var i,l,
        dict = vm.data.simpleData,
        key =  dict.idKey,
        parentKey =  dict.pIdKey,
        childKey ="children";
        ...
        if (Object.prototype.toString.apply(sNodes) === "[object Array]") {
            ...
            for (i=0, l=sNodes.length; i<l; i++) {
                if(sNodes[i][childKey])sNodes[i][childKey]=null;
                tmpMap[sNodes[i][key]] = sNodes[i];
            }
            if(vm.chkIds && Object.prototype.toString.apply(vm.chkIds) === "[object Array]" && vm.chkIds.length>0 ){
                for (i=0, l=vm.chkIds.length; i<l; i++) {
                    for (j=0, k=sNodes.length; j<k; j++) {
                        if(sNodes[j].id==vm.chkIds[i])
                        {
                            sNodes[j]["checked"]=true;
                            break;
                        }
                    }
                }
            ...
            for (i=0, l=sNodes.length; i<l; i++) {
                sNodes[i] = itemFormator(sNodes[i], undefine, vm);
    ...
    var widget = avalon.ui.tree = function(element, data, vmodels) {
        ...
        if(newOpt.data.simpleData.enable) {//@liubl 代码有修改
            newOpt.children = transformTozTreeFormat(newOpt.children, newOpt)
            newOpt.data.simpleData.enable = false
        }
      ...
      /*  if(newOpt.data.simpleData.enable) {//@liubl 代码有修改
        ...
        var vmodel = avalon.define(data.treeId, function(vm) {
             ...
             */
            vm.reset = function(children,chkIds) {
                vm.chkIds = [];
                if(chkIds && chkIds.length>0) vm.chkIds=chkIds      
                children =  transformTozTreeFormat(children,vm)
                vm.children.clear()
