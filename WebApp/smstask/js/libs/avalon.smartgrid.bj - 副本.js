...
define(function(require) {
    ...
    if(sysConfig.jsTracking) avalon.log('seajs-Tracking："smartgrid---表格组件"'); 
    require("A-common-css");
    require("A-smartgrid-css"); 

    var template = require("text!../libs/model/avalon.smartgrid.bj.html");
    ...
    var widget = avalon.ui.smartgrid = function (element, data, vmodels) {
        ...
        var vmodel = avalon.define(vmId, function (vm) {
            ...
            vm._setColumnWidth = function (resize) {
                var cells = vmodel._container.getElementsByTagName('tr')[0].cells, columns = vmodel.columns, _columns = columns.$model,  minColumnWidth = getMinColumnWidth(_columns), firstStringColumn = getFirstStringColumn(columns, vmodel);
                //var $gridContainer = avalon(vmodel.container),containerWidth = $gridContainer.width();
                var containerWidth = avalon(element).width();

                var $gridTableContainer =  avalon(getByClassName("oni-smartgrid", vmodel.container)[0]);
                var $gridScrollbarContainer =  avalon(getByClassName("oni-smartgrid-divscroller",  vmodel.container)[0]);
                if (minColumnWidth > containerWidth && !resize || !vm.autoResize) {
                    //$gridContainer.css('width', minColumnWidth);
                    $gridScrollbarContainer.css('width', containerWidth);
                    firstStringColumn.width = firstStringColumn.configWidth;
                } else {
                    // //$gridContainer.css('width', 'auto');
                    // $gridScrollbarContainer.css('width', 'auto');
                    // firstStringColumn.width = 'auto';
                }                
                $gridTableContainer.css({'width':minColumnWidth});
                ...
                //perfectColumns(options, $gridScrollbarContainer);
            };
            ...
            vm.$init = function () {
                ...
                gridFrame = gridHeader.replace('MS_OPTION_ID', vmodel.$id).replace('MS_OPTION_height', 500);
                container.innerHTML = gridFrame;
                dataFracte(vmodel)
                avalon.scan(container, vmodel);
                // var $gridTableContainer =  avalon(getByClassName("oni-smartgrid", vmodel.container)[0]);
                // $gridTableContainer.css({"height":500px});
                avalon.nextTick(function () {
                ...
                if (vmodel.isAffix) {
                    callbacksNeedRemove.scrollCallback = avalon(window).bind('scroll', function () {
                        var scrollTop = Math.max(document.body.scrollTop, document.documentElement.scrollTop), offsetTop = $element.offset().top, 
                        headerHeight = avalon(element.getElementsByTagName('thead')[0]).css('height'), top = scrollTop - offsetTop + vmodel.affixHeight, clientHeight = avalon(window).height(), tableHeight = $element.outerHeight(), _position = vmodel._position;
    ...
    function perfectColumns(options, element) {
        var columns = options.columns, selectColumn = {}, allColumnWidth = 0, maxWidth = 0, maxWidthColumn = {};
        var parentContainerWidth = avalon(element).width();
        //var parentContainerWidth =1000;
        for (var i = 0, len = columns.length; i < len; i++) {
